    ...
    useEffect(() => {
        const observer = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting) {
                const loop = () => {
                    setStep(0);
                    setTimeout(() => setStep(1), 1000); // Highlight Red
                    setTimeout(() => setStep(2), 2500); // Move Up
                    setTimeout(() => setStep(3), 3500); // Transform to Cyan
                    setTimeout(() => setStep(4), 4500); // Checkmark Pop
                };
                loop();
                const interval = setInterval(loop, 8000); // Loop full animation
    ...
    return (
        <div ref={ref} className="w-full h-56 bg-slate-50 rounded-xl border border-slate-200 overflow-hidden relative p-4 flex flex-col gap-3 group-hover:bg-white transition-colors">
            ...
            <div className="relative flex flex-col gap-3">
                {/* ITEM 1: Análise Combinatória (Initially Top, moves down) */}
                ...
                {/* ITEM 2: Química Orgânica (The Problem Child - Moves Up) */}
                ...
                {/* ITEM 3: Função 1º Grau (Static but gets checked) */}
...
const MockupTerminal = () => {
            ...
            if (entry.isIntersecting) {
                ...
                const progressInterval = setInterval(() => {
                    ...
                    if (p >= 100) {
                        ...
                        setLines(prev => [...prev, "> OCR Text Extraction... Complete."]);
                        
                        // Phase 2: Analysis (Typing effect)
                        setTimeout(() => setLines(prev => [...prev, "> Loading INEP Criteria... OK"]), 500);
                        setTimeout(() => setLines(prev => [...prev, "> Checking C1 (Grammar)... OK"]), 1200);
                        setTimeout(() => setLines(prev => [...prev, "> Checking C3 (Argument)..."]), 2000);
    ...
    return (
        <div ref={ref} className="bg-slate-950 rounded-xl shadow-2xl border border-slate-800 overflow-hidden font-mono text-xs relative hover:shadow-cyan-500/10 transition-shadow duration-500">
            {/* Terminal Header */}
            <div className="bg-slate-900 px-4 py-2 flex items-center gap-2 border-b border-slate-800">
                ...
                <span className="ml-2 text-slate-500 animate-pulse">analyzing_essay.png</span>
            </div>

            {/* Terminal Body */}
            <div className="p-4 h-64 flex flex-col relative" ref={ref}>
                
                {/* OCR Progress */}
                {!showResult && progress < 100 && (
                    <div className="mb-4">
                        <div className="flex justify-between text-cyan-500 mb-1">
                            <span>Scanning handwriting...</span>
                            <span>{progress}%</span>
                        </div>
                ...
                <div className="space-y-1 text-slate-300">
                    ...
                    {lines.length > 0 && !showResult && (
                        <span className="inline-block w-2 h-4 bg-cyan-500 animate-blink align-middle ml-1"></span>
                    )}
                </div>

                {/* Final Result Popup */}
                <div className={`mt-auto transition-all duration-700 transform ${showResult ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <div className="bg-slate-900 border border-green-500/30 p-4 rounded-lg shadow-[0_0_15px_rgba(34,197,94,0.2)] relative overflow-hidden group">
                        ...
                        <p className="text-slate-300 leading-relaxed">
                            &gt; Competência 1: <span className="text-white font-bold">160 pontos</span>. <br/>
                            <span className="text-red-400">&gt; Atenção à crase na linha 14.</span><br/>
                            <span className="text-red-400">&gt; Uso de "onde" inadequado na linha 21.</span>
                        </p>
                    </div>
                </div>
            </div>
...
const MockupSisu = () => {
    ...
    // Calculate percentage height for bars (normalize around 900 max)
    const maxScale = 900;
        ...
        const observer = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting) {
                ...
                const timer = setInterval(() => {
                    ...
                    // Update progress (0 to 1) for css transition
                    setProgress(stepCount / steps);
                    setCurrentScore(Math.floor(start * 10) / 10); // 1 decimal
...
// --- MOCKUP INTERACTIVE ESSAY (NEW REALISTIC DEMO) ---
const MockupInteractiveEssay = () => {
    ...
    useEffect(() => {
        const observer = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting) {
                // Loop: 0=Typing, 1=Scanning, 2=Marks, 3=Result
                const loop = () => {
                    setAnimationState(0);
                    setTimeout(() => setAnimationState(1), 1000); // Start Scan
                    setTimeout(() => setAnimationState(2), 2500); // Show Marks
                    setTimeout(() => setAnimationState(3), 4000); // Show Score
                };
                loop();
...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const [scrolled, setScrolled] = useState(false); // Navbar State
  ...
  // Teaser Modals
  const [showTRITeaser, setShowTRITeaser] = useState(false); // New state
  const [showCutoffTeaser, setShowCutoffTeaser] = useState(false); // New state

  // Tour States
  // const [showLandingTour, setShowLandingTour] = useState(false); // Removed for simplicity
  // const [mockupTab, setMockupTab] = useState<'redacao' | 'simulado' | 'cronograma'>('redacao'); // Removed for simplicity

  // Removed LANDING_TOUR_STEPS as the tour is removed from landing page
  ...
  const checkPasswordStrength = (pass: string) => {
      ...
      setPasswordStrength(score); // 0 to 4
  };
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      const mockGoogleUser: User = {
        ...
        avatar: 'https://ui-avatars.com/api/?name=Ana+Silva&background=06B6D4&color=fff',
        planType: 'FREE',
        hasSeenOnboarding: false, // New users haven't seen tour
        hasSeenOnboardingGoalSetter: false, // New users haven't set goals
        hasSeenEssayDemo: false, // New users haven't seen essay demo
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        const mockUser: User = {
            ...
            planType: 'FREE',
            hasSeenOnboarding: false, // Existing user might not have seen tour
            hasSeenOnboardingGoalSetter: false, // Existing user might not have set goals
            hasSeenEssayDemo: false, // Existing user might not have seen essay demo
            usage: { essaysCount: 0, lastEssayDate: null, examsCount: 0, lastExamDate: null, schedulesCount: 0, lastScheduleDate: null },
            tokensConsumed: 0
        };
        onLogin(mockUser);
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        const newUser: User = {
            ...
            planType: 'FREE',
            hasSeenOnboarding: false, // New user
            hasSeenOnboardingGoalSetter: false, // New user
            hasSeenEssayDemo: false, // New user
            usage: { essaysCount: 0, lastEssayDate: null, examsCount: 0, lastExamDate: null, schedulesCount: 0, lastScheduleDate: null },
            tokensConsumed: 0
        };
        onLogin(newUser);
  ...
  if (view === 'auth') {
      return (
        <div className="min-h-screen bg-white flex flex-col lg:flex-row font-sans animate-fade-in relative">
          ...
          {/* Left Side (Marketing Context for Auth) */}
             ...
             <div className="w-full max-w-md">
                {authMode === 'login' && (
                    <div className="animate-fade-in">
                        ...
                        <button onClick={handleGoogleLogin} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-3.5 px-4 rounded-xl transition-all mb-6 shadow-md transform active:scale-95">
                        ...
                        <div className="relative flex py-2 items-center mb-6">
                            ...
                            <span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-bold uppercase tracking-wider">ou email</span>
                            <div className="flex-grow border-t border-slate-200"></div>
                        </div>
                ...
                {authMode === 'register' && (
                    <div className="animate-fade-in">
                        ...
                        <div className="relative flex py-2 items-center mb-4">
                            <div className="flex-grow border-t border-slate-200"></div>
                            <span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-bold uppercase tracking-wider">ou cadastro clássico</span>
                            <div className="flex-grow border-t border-slate-200"></div>
                        </div>
  ...
  // --- RENDER: LANDING PAGE (FULL WIDTH) ---
  return (
    <div className="min-h-screen bg-slate-900 font-sans selection:bg-fuchsia-500 selection:text-white">
      ...
      {/* OnboardingTour component removed from landing page */}
      ...
      {/* OPTIMIZED HERO SECTION (CENTERED, NO MOCKUP) */}
      <section className="bg-slate-900 pt-32 pb-16 px-6 lg:px-16 relative overflow-hidden text-center"> {/* Added text-center here */}
             {/* Abstract blobs */}
             ...
             <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center"> {/* Centered content within max-width */}
                ...
                <div className="animate-fade-in-up w-full flex flex-col items-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        ...
                        {/* "Ver Tour Interativo" button removed */}
                    </div>
        ...
        {/* --- NEW SECTION: LEAD MAGNET TEASERS (MOVED UP) --- */}
        ...
        {/* PAIN POINTS SECTION (Interactive Solution Cards) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 flex flex-col hover:border-indigo-200">
                    ...
                    <div className="p-6 border-t border-slate-100 bg-white relative z-10">
                         {/* Removed group-hover/translate-y/opacity for always visible text */}
                         <div>
                ...
                {/* Solution Card 3 - Updated for SIMULADOS */}
        ...
        {/* NEW SECTION: SISU APPROVAL VISION */}
